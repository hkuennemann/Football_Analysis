...
class ViewTransformer():
    def __init__(self):
        court_width = 68
        court_length = 23.32 # four stripes of the field, not entire field

        # Trapazoid Verticies
        self.pixel_verticies = np.array([
            [110, 1035], # found by trial and error approach
            [265, 275],
            [910, 260],
            [1640, 915]
        ])

        # Target Verticies, i.e. rectangle verticies
        ...
        self.perspective_transformer = cv2.getPerspectiveTransform(self.pixel_verticies, 
                                                                   self.target_verticies)
        ...
        for object, object_tracks in tracks.items():
            for frame_num, track in enumerate(object_tracks):
                for track_id, track_info in track.items():
                    position = track_info['position_adjustment']
